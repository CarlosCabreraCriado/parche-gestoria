...
const pathToDbFolder = path.join( DEBUG ? __dirname : __dirname, DEBUG ? "" : "../", "db");
...
function inicializarMasterDB() {
    ...
    masterDB.loadDatabase(function(err) {
        if (err) {
            console.log(err);
            console.log(
                "Se ha producido un error cargando MasterDB."
            );
        }
    });
    ...
    //Verifica si existe:
    masterDB.find({ nombreId: configuracionMaestra.nombreId }, function(
        err,
        docs
    ) {
        if (err) {
        ...
        if (docs.length != 0) {
            ...
            masterDB.insert(configuracionMaestra, function(
                err,
                newDocs
            ) {
                if (err) {
        ...
        if (err) {
            ...
            auxFiles.forEach((file,index, array) => {
                ...
                if (file.indexOf(".") == -1) {
                    ...
                    archivoProyecto.loadDatabase(function(
                        err
                    ) {
                        if (err) {
                            console.log(err);
...
ipc.on("crearProyecto", function(event, dataProyecto) {
    ...
    if (!fs.existsSync(path.join(pathToDbFolder, nombreProyecto))) {
        ...
        fs.mkdirSync(
            path.join(pathToDbFolder, nombreProyecto, "Imagenes")
        );
        fs.mkdirSync(
            path.join(pathToDbFolder, nombreProyecto, "Archivos")
        );
        fs.mkdirSync(
            path.join(pathToDbFolder, nombreProyecto, "Informes")
        );
        fs.mkdirSync(
            path.join(pathToDbFolder, nombreProyecto, "Plantillas")
        );
        fs.mkdirSync(
            path.join(pathToDbFolder, nombreProyecto, "Programas")
        );
    }
        ...
        if (docs.length != 0) {
            ...
            proyecto.update(
                ...
                function(err, numReplaced, upsert) {
                    if (err) {
                        console.log(
                            "Se ha producido remplazando el archivo"
                        );
                        console.log(err);
                        event.returnValue = false;
                    } else {
...
ipc.on("abrirProyecto", function(event, nombreProy) {
    ...
    proyecto.find({nombreId:"proyectoConfig"}, function(err, docs) {
        if (err) {
            ...
            console.log(
                "Enviando documentos de base de datos: " +
                    nombreProy
            );
            console.log(docs);
    ...
    procesosGenerales = new ProcesosGenerales(pathToDbFolder, nombreProyecto, proyecto);
    procesosDocumentos = new ProcesosDocumentos(pathToDbFolder, nombreProyecto, proyecto);
    ...
    procesosDesarrollador = new ProcesosDesarrollador(pathToDbFolder, nombreProyecto, proyecto);
    procesosGoogle = new ProcesosGoogle(pathToDbFolder, nombreProyecto, proyecto);
    procesosDespacho = new ProcesosDespacho(pathToDbFolder, nombreProyecto, proyecto);
    ...
    //Creacion de instancia de ejecucion de procesos: 
    procesarPlantillaExcel = new PlantillaExcel(pathToDbFolder, nombreProyecto, proyecto);
    procesarPlantillaDocx = new PlantillaDocx(pathToDbFolder, nombreProyecto, proyecto);
...
ipc.on("getDocumentoPath", function(event, pathDato, nombreArchivo, filtro) {
    ...
    console.log("Path:");
    console.log(path.join(pathToDbFolder,nombreProyecto, pathDato , nombreArchivo+".db"));
    console.log("Filtro:");
    console.log(filtro);
...
ipc.on("getListaObjetosEnColeccion", function(event, pathDato, nombreArchivo) {
    ...
    console.log( "Obteniendo objetos en coleccion: " + nombreProyecto + "path: " + path);
    ...
    archivoCargado.find({},{nombreId: 1,objetoId: 1, _id: 0}, function(err, docs) {
        if (err) {
            console.log("Se ha producido un error");
            console.log(err);
            event.returnValue = null;
...
ipc.on("getObjetoEnColeccion", function(event, pathDato, nombreArchivo, objetoId) {
    ...
    archivoCargado.find({objetoId: objetoId},{data: 1, _id: 0}, function(err, docs) {
        if (err) {
            console.log("Se ha producido un error");
            console.log(err);
            event.returnValue = null;
...
ipc.handle("onImportarSpool", async (event, rutaInput, nombreOutput ) =>{
    ...
    const pathSpoolOutput = path.join(pathToDbFolder, nombreProyecto,"/archivos",nombreOutput+".txt");
...
ipc.handle("onEjecutarProceso", async (event, proceso, argumentos) =>{
    ...
    switch(categoria){
        ...
        case "spool":
            ...
            break;
        case "remedy":
            result = await procesosRemedy[identificador](argumentos.formularioControl);
            break;
        case "desarrollador":
            result = await procesosDesarrollador[identificador](argumentos.formularioControl);
            break; 
        case "google":
            result = await procesosGoogle[identificador](argumentos.formularioControl);
            break; 
        case "import":
        ...
        case "kPIs":
            ...
            break; 
        case "despacho":
            result = await procesosDespacho[identificador](argumentos.formularioControl);
            break; 
    }
...
ipc.handle("onEjecutarPlantilla", async (event, proceso, argumentos) =>{
    ...
    for(var i=0; i<proceso.argumentos.length;i++){
        datosPlantilla.data[proceso.argumentos[i].identificador] = proceso.argumentos[i].formulario
        console.log(proceso.argumentos[i].formulario)
    }

    const template = fs.readFileSync('/Users/carloscabreracriado/Desktop/simple.docx');
...
ipc.handle("onProcesarSpool", async (event, datos) =>{
    ...
    const pathSpoolInput = path.join(pathToDbFolder, nombreProyecto,"spoolInput.txt");
    const pathSpoolOutput = path.join(pathToDbFolder, nombreProyecto,"spoolOutput.txt");
...
ipc.handle("onGetArbolProyecto", async (event, nombreProyectoArg) =>{
        ...
        return new Promise((resolve)=>{
            ...
            fs.readdir(rutaDirectorio, async (err, files) => {
                if (err) {
                    ...
                    files.forEach((file)=> {
                        if(file.indexOf(".")!=-1){
                            nivelArbol.push({nombre: file,tipo: file.substr(file.lastIndexOf('.') + 1), direccion: rutaDirectorio})
                        }
                    })
                    ...
                    files.forEach((file)=> {
                        if(file.indexOf(".")==-1){
                            nivelArbol.push({nombre: file,tipo: "dir", direccion: rutaDirectorio, subDirectorio: null})
                        }
                    })
    ...
    async function  explorarDirectorio(ruta, nivelArbol=null){
        ...
        for(var i = 0; i<indicesDirectorioSinExplorar.length; i++){
            nivelArbol[indicesDirectorioSinExplorar[i]].subDirectorio = await explorarDirectorio(path.join(ruta,nivelArbol[indicesDirectorioSinExplorar[i]].nombre))
        }
    ...
    arbolProyecto= await explorarDirectorio(path.join(pathToDbFolder, nombreProyectoArg));
...
ipc.handle("onGuardarDocumento", async (event, datos, tipo) =>{
    ...
    if(tipo != "archivos" && tipo != "plantillas"){
        console.log("Se ha producido un error en el guardado de documento: Tipo incorrecto.")
        return false;
    }

    console.log("Guardando Documento: "+datos.nombreId);
    //Verifica que existe un proyecto abierto:
    if(nombreProyecto=="" || nombreProyecto==null || nombreProyecto== undefined){
            ...
            if (docs.length != 0) {
                ...
                documento.update(
                    ...
                    function(err, numReplaced, upsert) {
                        if (err) {
                            console.log(
                                "Se ha producido un error remplazando el archivo"
                            );
                            console.log(err);
                            resolve(false);
                            return false;
...
async function guardarDocumento(datos){
    ...
    var documento = new Datastore(
        path.join(pathToDbFolder, nombreProyecto,"archivos",datos.nombreId+".db")
    );
...
function autentificarGoogle(url){
 ...
 module.exports = {autentificarGoogle,guardarDocumento,mostrarError,mostrarWarning}
