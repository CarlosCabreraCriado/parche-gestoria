...
export class DashboardComponent implements OnInit{
    ...
    constructor(public appService: AppService, public dashboardService: DashboardService, public dialog: MatDialog) {
    ...
    @ViewChild("AddCursoComponent",{static: false}) addCursoComponent: AddCursoComponent;
    ...
    addDato(opcionesArg:any){
        const dialogRef = this.dialog.open(AddDatoComponent,{
            disableClose: true,
            width: "50%",
            data: {opciones: opcionesArg, titulo:"Añadir dato",contenido:"Aqui se añaden datos."}
        });
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            if(result=="error"){
                const dialogExito = this.dialog.open(DialogoComponent,{
                    data: {tipoDialogo: "error", titulo:"Se ha producido un error.",contenido:"No se ha podido guardar el archivo debido a un error inesperado."}
                });
            }
    ...
    gestionarDato(){
        const dialogRef = this.dialog.open(GestionarDato,{
            disableClose: true,
            width: "70%",
            data: {opciones: {}, titulo:"Gestionar datos",contenido:"Panel de gestion de datos"}
        });
    ...
    visualizarDato(){
        ...
        const dialogRef = this.dialog.open(VisualizarDato,{
            disableClose: true,
            width: "70%",
            data: {opciones: {}, titulo:"Visualizador de datos",contenido:"Panel de visualización de datos"}
        });
    ...
    insertarElemento(opcionesArg:any){
        ...
        switch(opcionesArg.tipo){
            case "texto":
                configuracionDialogo = {
                    disableClose: true,
                    width: "50%",
                    data: {opciones: opcionesArg, titulo:"Insertar Elemento",contenido:this.elementosRender}
                }
                break;
            case "tabla":
                configuracionDialogo = {
        ...
        const dialogRef = this.dialog.open(InsertarElementoComponent,configuracionDialogo);
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            if(result=="error"){
                const dialogExito = this.dialog.open(DialogoComponent,{
                    data: {tipoDialogo: "error", titulo:"Se ha producido un error.",contenido:"No se ha podido insertar el elemento."}
                });
            }
