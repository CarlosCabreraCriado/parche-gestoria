...
function writeLeeme(sheet, fechaGen, inputName, specsSel, verbatimSel) {
  ...
  const lines = [
    ...
    ["  Nº EMPLEADOS", "Nº de trabajadores de la empresa. Solo lo usa la ejecución ANUAL, y solo para los conceptos de precio escalado (modelo 190 y certificados de retención): es la cantidad con la que se entra en la tabla de tramos de la hoja 'Modelo 190' del archivo de mapeos. En las ejecuciones trimestrales y mensuales se ignora, y en la anual solo la mira la hoja 111: el modelo 180 de la hoja 115 va a tarifa fija y no la necesita. Vacío NO es lo mismo que 0: vacío significa que falta el dato y la línea sale en incidencias; 0 es una empresa sin trabajadores, que presenta el 190 igual y entra en el primer tramo. OJO: la plantilla se reutiliza entre ejercicios — revisa este número cada cierre anual. Cada ejecución lista el tramo y el precio aplicados a cada línea en precios_escalados.csv."],
    ["", ""],
    ["LA EJECUCIÓN ANUAL (modelos 190 y 180)", "", true],
    ["  Los resúmenes anuales no tienen hoja ni filas propias: los presentan las mismas empresas que ya declaran su modelo periódico, así que se facturan desde las hojas existentes SIN tocarlas. Al elegir Frecuencia = Anual en el formulario: (1) entran las filas TRIMESTRAL y MENSUAL además de las ANUAL, porque un resumen anual lo presenta tanto quien declaró trimestralmente como quien lo hizo mes a mes — no hay que cambiar ninguna FRECUENCIA en la plantilla; (2) cada fila de la hoja 111 con FACTURAR=SI genera DOS líneas: el modelo 190 y su certificado de retenciones, cada uno con su precio según el Nº EMPLEADOS; (3) cada fila de la hoja 115 con FACTURAR=SI genera UNA línea: el modelo 180. No lleva certificado asociado y va a tarifa fija, así que su precio no depende del Nº EMPLEADOS y esa columna no hace falta en la hoja 115; (4) las filas del resto de hojas (130, 420, 202…) no se facturan, porque esos modelos no tienen resumen anual.", ""],
    ["", ""],
