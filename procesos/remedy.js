...
class ProcesosRemedy {
	...
	async extraccionRemedy(argumentos){
		...
		//
			...
			await page.goto('https://oneitsm.onbmc.com/arsys/forms/onbmc-s/AR+System+Report+Console/Default+Administrator+View/');
			...
			await page._client.send('Page.setDownloadBehavior', {behavior: 'allow', downloadPath: '/Users/carloscabreracriado/OneDrive - Vodafone Group/Procesamiento'});	
		...
		//	Seleccion de reporte:
			await page.waitForTimeout(tiempoEspera);
			await page.waitForSelector("#T93250 > tbody > tr:nth-child(2) > td:nth-child(2) > nobr > span")
			await page.click("#T93250 > tbody > tr:nth-child(2) > td:nth-child(2) > nobr > span")
		...
		//Ejecución de reporte:
			...
			await page.waitForTimeout(tiempoEspera);
			await frame.waitForSelector('#toolbar > table > tbody > tr:nth-child(2) > td:nth-child(5) > input');
			await frame.click("#toolbar > table > tbody > tr:nth-child(2) > td:nth-child(5) > input")
	...
	async procesarReportRemedy(argumentos){
		...
		//PROCESAMIENTO: 
		return await XlsxPopulate.fromFileAsync(path.normalize(pathInformeIncidencias))
			.then(workbook => {
			...
			.then(()=>{
				...
				numColumnasInforme = informeIncidencias.sheet("DB-Historial").usedRange()._numColumns;
				numFilasInforme = informeIncidencias.sheet("DB-Historial").usedRange()._numRows;
				...
				//MAPEO DE CABECERAS:
				var cabecerasInforme = ["Fecha Cambio","Hora Cambio","Incident Number","Servicio RMCA","Service","Assigned Group","Submit Date","Status","Hub Actual","Prioridad","Summary","Descripcion","Solicitante","Estado","PROCESO RMCA","Impacto","Responsable","W.A","Descripción WA","CRQ","FECHA CRQ","TARGET DATE","Submit week","Fecha cierre"]
				...
				for(var i = 0; i<reportIncidencias[0].data.length; i++){
					...
					for(var j = 0; j<numFilasInforme; j++){
						...
						if(incidenciaAnalizada==hojaHistorial.row(j+1).cell(3).value()){
							...
							for(var k=3; k<numColumnasInforme; k++){
								if(hojaHistorial.row(j+1).cell(k).value()){
									estadoActualIncidencia[k-3]= hojaHistorial.row(j+1).cell(k).value()
								}
							}
					...
					if(incidenciaEncontrada){
						...
						for(var j=0; j<mapeoHistorialReport.length; j++){
							...
							if(estadoActualIncidencia[j]!= reportIncidencias[0].data[i][mapeoHistorialReport[j]] && mapeoHistorialReport[j] != "" && reportIncidencias[0].data[i][mapeoHistorialReport[j]] != undefined){
								console.log("CABECERA "+ mapeoHistorialReport[j])
								console.log("CAMBIO "+ reportIncidencias[0].data[i][mapeoHistorialReport[j]])
								console.log("Marcando Fecha/Hora cambio:");
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(1).value(moment().toDate())
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(2).value(time)
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(3).value(reportIncidencias[0].data[i]["incident_id*+"])
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(3+j).value(reportIncidencias[0].data[i][mapeoHistorialReport[j]])
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(26).value("Modificado")
					...
					for(var j=0; j<estadoActualIncidencia.length; j++){
						if(estadoActualIncidencia[j]!="" || ( reportIncidencias[0].data[i]["status*"] == "Resolved") ||  (reportIncidencias[0].data[i]["service*+"] == undefined )){
					...
					if(incidenciaNueva){
						...
						for(var  prop in reportIncidencias[0].data[i]){
							...
							if(indiceColumna!=0){
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(indiceColumna).value(reportIncidencias[0].data[i][prop])
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(26).value("Entrada")
							}
					...
					if(incidenciaEncontrada == false){console.log("No ENCONTRADO") }
				}
				...
				for(var i = 1; i<numFilasInforme; i++){
					incidenciaAnalizada = hojaHistorial.row(i+1).cell(3).value()
					incidenciaEncontrada= false;
					if(incidenciaAnalizada == undefined) continue;
					if(reportIncidencias[0].data.find(i=> i["incident_id*+"]==incidenciaAnalizada)){
						incidenciaEncontrada=true;
					}else{
						if(incidenciasCerradas.indexOf(incidenciaAnalizada)<0){
							console.log("No encontrada: " + incidenciaAnalizada + " Marcanco 'Cancelled/Closed'")
				...
				for(var i=0; i<incidenciasCerradas.length;i++){
					...
					for(var j = 1; j<=numFilasInforme; j++){
						...
						if(informeIncidencias.sheet("DB-Historial").row(j).cell(3).value() == incidenciasCerradas[i] && informeIncidencias.sheet("DB-Historial").row(j).cell(9).value() != ""){
							estadoFinalIncidencia = informeIncidencias.sheet("DB-Historial").row(j).cell(9).value();
						}
					}
				...
				for(var i = 0; i<incidenciasCerradas.length; i++){
					informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1+i).cell(1).value(moment().toDate())
					informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1+i).cell(2).value(time)
					informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1+i).cell(3).value(incidenciasCerradas[i])
					informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1+i).cell(9).value("Cancelled/Closed")
					informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1+i).cell(26).value("Salida")
					registrosCerrados++;
				}
	...
	async procesarExtraccionPowerBI(argumentos){
		...
		for (var j = 1; j <= numeroCabecerasPowerBI; j++) {
			cabecerasPowerBi.push(excelReportPowerBi.sheet("Data_Backlog").cell(1,j).value())
		}

		for (var i = 2; i <= numeroRegistrosPowerBI; i++) {
			objetoIncidencia = {}	
			for (var j = 1; j <= numeroCabecerasPowerBI; j++) {
				objetoIncidencia[cabecerasPowerBi[j-1]] =excelReportPowerBi.sheet("Data_Backlog").cell(i,j).value()
			}
		...
		for (var i = 0; i <= reportIncidencias[0].data.length; i++) {
			...
			if(reportIncidencias[0].data[i]){
				if(reportIncidencias[0].data[i]["Service"] != "VFES-SAP ECC 6 0 RMCA-PROD"){
		...
		return await XlsxPopulate.fromFileAsync(path.normalize(pathInformeIncidencias))
			...
			.then(()=>{
				...
				//var cabecerasReport = ["Incident ID*","Service Request ID","Last Name+","First Name","Summary*","Service*+","Priority*","Status*","Assigned Group*+","Assignee+","Target Date","Incident Type*","Submitter*","Submit Date","operational_categorization_tier_3"]
				var cabecerasReport = ['Incident Number','Support Group Name','Submit Date','Summary','Priority','Service Type','Assigned Group','Status','Status Reason','Cause','Estimated Resolution Date','Required Resolution Datetime','Service','Closed Date','Last Resolved Date','Last Modified Date','Categorization Tier 1','Categorization Tier 2','Categorization Tier 3','Product Categorization Tier 1','Product Categorization Tier 2','Product Categorization Tier 3','Reported Source','Closure Product Name','Environment','Type','SPIRIT','Antiguëdad','Rango','Estado','Severity','Inc. Number','Asignado','Group','Group Creator','Product Resp','Product Resp II','Cat','Antiguëdad 2','Antigüedad modificacion','Rango 2','Groups Number','Stack','Inc_Type','Revised_Type']

				//Mapeo:
				//var mapeoHistorialReport = ["incident_id*+","incident_type*","service*+","assigned_group*+","assignee+","submit_date","status*","operational_categorization_tier_3","priority*","summary*","","","","","","","","","","","target_date"]
				var mapeoHistorialReport = ["Incident Number","Type","Service","Assigned Group","Asignado","Submit Date","Status","Categorization Tier 3","Priority","Summary","","","","","","","","","","","Estimated Resolution Date"]
					...
					if(incidenciaEncontrada){
						...
						for(var j=0; j<mapeoHistorialReport.length; j++){
							...
							if(estadoActualIncidencia[j]!= reportIncidencias[0].data[i][mapeoHistorialReport[j]] && mapeoHistorialReport[j] != "" && reportIncidencias[0].data[i][mapeoHistorialReport[j]] != undefined){
								...
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(1).value(moment().toDate())
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(2).value(time)
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(3).value(reportIncidencias[0].data[i]["Incident Number"])
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(3+j).value(reportIncidencias[0].data[i][mapeoHistorialReport[j]])
								informeIncidencias.sheet("DB-Historial").row(numFilasInforme+1).cell(26).value("Modificado")
								incidenciaModificada = true;
				...
				for(var i = 1; i<numFilasInforme; i++){
					...
					incidenciaEncontrada= false;
					if(incidenciaAnalizada == undefined) continue;
					if(reportIncidencias[0].data.find(i=> i["Incident Number"]==incidenciaAnalizada)){
						incidenciaEncontrada=true;
					}else{
						if(incidenciasCerradas.indexOf(incidenciaAnalizada)<0){
							console.log("No encontrada: " + incidenciaAnalizada + " Marcanco 'Cancelled/Closed'")
	...
	async renderizarReportRemedy(argumentos){
		...
		return await XlsxPopulate.fromFileAsync(path.normalize(pathInformeIncidencias))
			...
			.then(()=>{
				...
				var hojaIncidenciasCerradas = informeIncidencias.sheet("INC resueltas-cerradas")

				var numColumnasHistorial = informeIncidencias.sheet("DB-Historial").usedRange()._numColumns;
				var numFilasHistorial = informeIncidencias.sheet("DB-Historial").usedRange()._numRows;

				var numColumnasAbiertas = informeIncidencias.sheet("INC abiertas").usedRange()._numColumns;
				var numFilasAbiertas = informeIncidencias.sheet("INC abiertas").usedRange()._numRows;

				var numColumnasCerradas = informeIncidencias.sheet("INC resueltas-cerradas").usedRange()._numColumns;
				var numFilasCerradas = informeIncidencias.sheet("INC resueltas-cerradas").usedRange()._numRows;
				...
				for(var i=1; i<numFilasAbiertas; i++){
					for(var j=1;j<numColumnasAbiertas;j++){
						hojaIncidenciasAbiertas.row(i+1).cell(j).clear()		
					}
				}
	...
	async renderizarBacklog(argumentos){
		...
		return await XlsxPopulate.fromFileAsync(path.normalize(pathInformeIncidencias))
			...
			.then(()=>{
				...
				var numColumnasBacklog = informeIncidencias.sheet("DB-Backlog").usedRange()._numColumns;
				var numFilasBacklog = informeIncidencias.sheet("DB-Backlog").usedRange()._numRows;
				...
				for( var i = 0; i<numFilasHistorial; i++){
					listaIncidencias.push(hojaHistorial.row(2+i).cell(3).value());
				}
				...
				for( var i = 0; i<numFilasBacklog; i++){
					hojaIncidenciasBacklog.row(i+2).cell(1).value("")
					hojaIncidenciasBacklog.row(i+2).cell(2).value("")
					hojaIncidenciasBacklog.row(i+2).cell(3).value("")
				}
				...
				for( var i = 0; i<incidenciasUnicas.length; i++){
					...
					for( var j = 0; j<numFilasHistorial; j++){
						...
						if(hojaHistorial.row(2+j).cell(3).value()== incidenciasUnicas[i]){

							//Actua en Entrada:
							if(hojaHistorial.row(2+j).cell(26).value() == "Entrada" && !flagSalida){
								console.log("Entrada:")
								console.log("INC: "+incidenciasUnicas[i])
								console.log(hojaHistorial.row(2+j).cell(1).value());
								var fechaExcel = Number(hojaHistorial.row(2+j).cell(1).value())
								fechaUltimaEntrada = moment(ExcelDateToJSDate(fechaExcel))
								console.log(fechaUltimaEntrada)
								hojaIncidenciasBacklog.row(cuentaFila).cell(2).value(fechaUltimaEntrada.format("DD/MM/YYYY"))
								hojaIncidenciasBacklog.row(cuentaFila).cell(1).value(incidenciasUnicas[i])

							}

							//Actua en Salida:
							if(hojaHistorial.row(2+j).cell(26).value() == "Salida" && !flagSalida){
								console.log("Salida");
								console.log("INC: "+incidenciasUnicas[i])
								console.log(hojaHistorial.row(2+j).cell(1).value());
								var fechaExcel = Number(hojaHistorial.row(2+j).cell(1).value())
								...
								hojaIncidenciasBacklog.row(cuentaFila).cell(3).value(fechaUltimaSalida.format("DD/MM/YYYY"))
								hojaIncidenciasBacklog.row(cuentaFila).cell(1).value(incidenciasUnicas[i])
								flagSalida= true;
							...
							if(flagSalida && (hojaHistorial.row(2+j).cell(26).value() == "Modificado" || hojaHistorial.row(2+j).cell(26).value() == "Modificación Manual")){
